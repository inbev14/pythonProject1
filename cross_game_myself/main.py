...
from cross_game_myself import assets
from cross_game_myself.game_lib.labels.pause import PauseLabel
from cross_game_myself.game_lib.labels.scoreboard import Scoreboard
from cross_game_myself.game_lib.manager.car_manager import CarManager
from cross_game_myself.game_lib.sprites.player import Player
from cross_game_myself.game_lib.windows.enter_name import EnterNameWin
from cross_game_myself.game_lib.windows.main import Window
...
class Game:
    ...
    def __init__(self):
        if self.begin_game():
            ...
            self.engine_run_sound = pygame.mixer.Sound(os.path.join(self.PATH_TO_ASSETS, 'sound', 'engine_run.wav'))
            self.finish_sound = pygame.mixer.Sound(os.path.join(self.PATH_TO_ASSETS, 'sound', 'finish.wav'))
            self.lose_sound = pygame.mixer.Sound(os.path.join(self.PATH_TO_ASSETS, 'sound', 'lose.wav'))
            self.pause_sound = pygame.mixer.Sound(os.path.join(Game.PATH_TO_ASSETS, 'sound', 'lose2.wav'))
            self.choosing_song = pygame.mixer.Sound(os.path.join(self.PATH_TO_ASSETS, 'sound', 'choosing.wav'))
            
            login_window = EnterNameWin()
            self.login = login_window.login.title()
            
            self.window = Window()
            self.window.canvas.bgpic(self.PATH_TO_BGPIC)
