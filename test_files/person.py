...
class PersonVerify:
    ...
    def verify_full_name(cls, full_name: str):
        ...
        for name in full_name:
            if len(name.strip(cls.LETTERS)) != 0:
                raise TypeError('Full name can only contain letters')
    ...
    def verify_age(cls, age: int):
        ...
        if age not in cls.AGE_RANGE:
            raise ValueError(f'Age must be between {cls.AGE_RANGE[0]} and {cls.AGE_RANGE[-1]} years old')
